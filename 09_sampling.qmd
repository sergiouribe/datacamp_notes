...
mean_body_mass_2000_samp <- replicate(
  # Use 2000 replicates
  n = 2000,
  expr = {
    # Start with the population
    penguins %>%
      filter(!is.na(body_mass)) %>%  # remove NAs to avoid errors
...
# See the result
mean_body_mass_2000_samp
```

```{r}
# Calculate the true population mean popularity
pop_mean <- mean(spotify_population$popularity)


# Calculate the original sample mean popularity
samp_mean <- mean(spotify_sample$popularity)


# Calculate the sampling dist'n estimate of mean popularity
samp_distn_mean <- mean(sampling_distribution$sample_mean)


# Calculate the bootstrap dist'n estimate of mean popularity
boot_distn_mean <- mean(bootstrap_distribution$resample_mean)
